...
for ((i=0; i<$NUM_CONTAINERS; i++)); do
    ...
    case $TYPE in
        0)
            ...
            docker run -d --name "$CONTAINER_NAME" --memory="128m" "${IMAGES[0]}" stress --vm 1 --vm-bytes 64M --timeout 60s
            ;;
        1)
            echo "Creando contenedor de CPU (moderado): $CONTAINER_NAME"
            docker run -d --name "$CONTAINER_NAME" --cpus="0.5" "${IMAGES[0]}" stress --cpu 1 --timeout 60s
            ;;
        2)
            echo "Creando contenedor de I/O (moderado): $CONTAINER_NAME"
            docker run -d --name "$CONTAINER_NAME" "${IMAGES[0]}" stress --io 2 --timeout 60s
            ;;
        3)
            echo "Creando contenedor de Disco (moderado): $CONTAINER_NAME"
            docker run -d --name "$CONTAINER_NAME" "${IMAGES[0]}" stress --hdd 1 --hdd-bytes 64M --timeout 60s
            ;;
    esac
done
